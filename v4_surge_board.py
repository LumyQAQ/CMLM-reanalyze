...
# ==========================================
# 🎨 页面排版与数据加载模块 (纯读取，无任何抓取逻辑)
# ==========================================
@st.cache_data(ttl=60)  # 缓存 60 秒，防止频繁刷新
def load_data(path):
...
trade_date = surge_date if surge_date != "等待数据同步..." else pb_date
...
with tab1:
    ...
    if not df_surge.empty:
        ...
        st.info("📉 暂无数据。请在本地运行引擎后推送到 GitHub。")
...
with tab2:
    ...
    if not df_pullback.empty:
        ...
        st.info("📉 暂无回踩标的。请在本地运行引擎后推送到 GitHub。")