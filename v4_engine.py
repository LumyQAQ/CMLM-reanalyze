...
def get_volume_factor():
    ...
    # 09:30 - 11:30
        ...
        if elapsed == 0: elapsed = 1  # 防除零
        return 240.0 / elapsed
    # 11:30 - 13:00 (中午休市，固定 2 倍)
...
def run_v4_engine():
    print("🚀 启动 CMLM V4.4 终极引擎 (全市场满血版 + 盘中量能外推雷达)...")
    ...
    if vol_factor > 1.0:
        print(f"🕒 [系统状态] 当前处于盘中/休市阶段，已自动开启量能外推算法！(当前放大系数: {vol_factor:.2f}x)")
    else:
        print("🕒 [系统状态] 当前为盘后静态复盘模式，量能系数为 1.0x。")
            ...
            if isinstance(df_k, pd.DataFrame) and not df_k.empty:
                ...
                if len(df_k) >= 20:
                    ...
                    if breakout_idx is not None:
                        ...
                        # 🔮 核心外推逻辑应用：如果盘中运行，必须用折算后的预估量去判定“是否缩量”
                        est_today_amt = today_amt * vol_factor
                        ...
                        if is_debug:
                            print(
                                f"🎯 [回踩诊断] {name}({code}): 距爆发 {len(df_k) - 1 - breakout_idx}天. 缩量:{vol_shrink}(预估比值{est_today_amt / breakout_amt:.2f}), 下跌:{price_drop}, 支撑:{holding_support}")
